...
推荐使用安装脚本。脚本会自动识别系统架构，优先从 ISCAS 镜像下载最新稳定版，并安装到 ``/usr/local/bin/ruyi``：
...
<CodeBlock lang="bash" showTitleCopyButton="true" code={`
$ curl --proto '=https' --tlsv1.2 -fsSL https://ruyisdk.org/install.sh | sh -s -- --install-dir "$HOME/.local/bin"
`} />
